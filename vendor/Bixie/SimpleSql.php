...
class SimpleSql {
	 ...
	 */
	public function __construct ($host, $username, $password, $database, $prefix = '', $driver = 'mysql') {
		$this->prefix = $prefix;
		// handle connection
	...
	public function query ($sql, $fetch_mode = \PDO::FETCH_ASSOC) {
		...
		do {
			...
			try {
				...
				$this->stmt = $this->pdo->query($this->sql, $fetch_mode);
				return $this->stmt;
	...
	public function exec ($sql) {
		...
		do {
			...
			try {
				...
				$this->stmt = $this->pdo->exec($this->sql);
				return $this->stmt;
	...
	public function insert ($table, array $data) {
		...
		$sql = 'INSERT INTO ' . $this->prefixSql($table) . ' SET ';
	...
	public function update ($table, array $data, $where = array()) {
		...
		// generate SQL
		$sql = 'UPDATE ' . $this->prefixSql($table) . ' SET ';
	...
	public function delete ($table, array $where = array()) {
		...
		// generate SQL
		$sql = 'DELETE FROM ' . $this->prefixSql($table) . ' WHERE ';
